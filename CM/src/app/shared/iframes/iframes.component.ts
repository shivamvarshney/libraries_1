        ...
        this.iframensubscribe = this.saleService.getTranslation().subscribe(res => {
          ...
          data.side_nav_text.map((navInfo, navKey) => {
            if (navInfo.externalData == type) { 
              this.ifarmeUrl = AppConstants.userManagementBaseUrl + navInfo.externalFor + '?sso-token=' + appToken + '&sso-source=' + iframe+ '&sso-client=' +AppConstants.clientId + '&sso-serviceId=' +AppConstants.serviceId;
