...
export class FreelanceAgentsDataProvider implements OnDestroy, AafGridDataProvider<AssignedKit>{
    paramUnsubscribe: Subscription;
    statusUnsubscribe: Subscription;
    constructor(
    ...
    getData(objInfo: any) {
        ...
        this.paramUnsubscribe = this.activatedRoutes.queryParams.subscribe((params:Params)=>{
            if(params.roleId && params.roleId != ''){
                urlString += `&roleId=${params.roleId}`; 
            }            
        });
        this.statusUnsubscribe = this.activatedRoutes.params.subscribe((params: Params) => {                        
            if (params.status) {
    ...
    ngOnDestroy() {
        ...
        this.statusUnsubscribe ? this.statusUnsubscribe.unsubscribe() : '';
      }
}