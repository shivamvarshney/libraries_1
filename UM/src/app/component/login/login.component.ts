...
export class LoginComponent implements OnInit, OnDestroy {
  ...
  onResetTimerHandler(resetTimer) {
    ...
    let serverResetDate = new Date(resetTimer.result.resetAt);
    let getResetMiliSec = serverResetDate.getTime();
    let convertResetServerMiliSec = this.convertMS(getResetMiliSec);
    let serverCurrentDate = new Date(resetTimer.result.currentTime);
    let getStartMiliSec = serverCurrentDate.getTime();
    let convertStartServerMiliSec = this.convertMS(getStartMiliSec);
    this.isDisabled = true;
    this.hideOTPTimer = true;
    this.timer = 59;
    this.minutes = (convertResetServerMiliSec.minute - 1) - convertStartServerMiliSec.minute;
    if (((convertResetServerMiliSec.minute > 55) && (convertResetServerMiliSec.minute < 60)) || (convertStartServerMiliSec.minute > 55) && (convertStartServerMiliSec.minute < 60)) { this.minutes = 4; }
    clearInterval(this.interval);
    this.interval = setInterval(() => {
      if (this.timer > '0') {
        this.timer--;
        convertStartServerMiliSec.hour > convertResetServerMiliSec.hour ? clearInterval(this.interval) : '';
      ...
      if ((this.timer === 0) && (this.minutes === 0)) {
        clearInterval(this.interval);
        this.minutes = 0;
        this.timer = '00';
        this.hideOTPTimer = false;
